    ...
    dummy_entries = [
        {
            ...
            "content": "Test note",
            "mood": "good",
            "created_at": "2024-05-29T12:00:00",
            "updated_at": "2024-05-29T12:00:00",
        }
    ]
    monkeypatch.setattr(
        "app.services.EntryService.get_entries", lambda: dummy_entries
    )
...
def test_get_entry(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.EntryService.get_entry", lambda d: dummy_entry
    )
...
def test_create_or_update(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.EntryService.create_or_update", lambda entry: dummy_entry
    )
...
def test_stats(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.EntryService.get_stats", lambda: dummy_stats
    )
