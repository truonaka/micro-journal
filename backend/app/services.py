...
class EntryService:
    @staticmethod
    def _as_entry_out(entry) -> EntryOut:
    ...
    def get_entries() -> List[EntryOut]:
        ...
        return [
            EntryService._as_entry_out(entry)
            for entry in EntryRepository.get_entries()
        ]

    @staticmethod
    def get_entry(entry_date: date) -> Optional[EntryOut]:
        logger.info("Fetching entry for date: %s", entry_date)
        entry = EntryRepository.get_entry(entry_date)
        if entry is None:
            return None
        return EntryService._as_entry_out(entry)

    @staticmethod
    def create_or_update(entry: EntryCreate) -> EntryOut:
        logger.info(
            "Create or update entry for date: %s, mood: %s",
            entry.date,
            entry.mood,
        )
        if entry.date != date.today():
            logger.warning(
                "Attempt to create or edit past entry: %s", entry.date
            )
            raise HTTPException(400, "Cannot edit past entries")
        result = EntryRepository.create_or_update(entry)
        logger.info(
            "Entry for %s saved (id: %s)",
            entry.date,
            getattr(result, "id", None),
        )
        return EntryService._as_entry_out(result)
